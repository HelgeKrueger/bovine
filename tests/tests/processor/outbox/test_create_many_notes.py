...
async def test_create_many_notes(blog_test_env):  # noqa F811
    ...
    first_page_result = await blog_test_env.get(result_json["first"])
    ...
    second_page_result = await blog_test_env.get(first_page_result["next"])
    ...
    third_page_result = await blog_test_env.get(second_page_result["next"])
