...
async def userinfo(account_name) -> dict:
...
async def inbox(account_name) -> dict:
    ...
    local_user = current_app.config.data_store.get_user(account_name)
...
async def outbox(account_name) -> dict:
    domain = current_app.config["DOMAIN"]
