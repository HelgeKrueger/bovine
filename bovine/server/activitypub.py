...
async def userinfo(account_name: str) -> dict:
    ...
    if not await current_app.config.validate_signature(request, digest=None):
        logger.warn("Invalid signature on get http request for account")
        return
...
async def handle_inbox(data):
    ...
    if not await current_app.config.validate_signature(request, digest=digest):
        logger.warn("Incorrect http signature on post")
...
async def outbox(account_name: str) -> dict:
    ...
    if not await current_app.config.validate_signature(request, digest=None):
        logger.warn("Invalid signature on get http request for outbox")
        return
