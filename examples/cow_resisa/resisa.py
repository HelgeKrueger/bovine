...
async def update_account(session, config):
    feed = feedparser.parse(config["feed_url"])

    entries = [entry_to_note(config["account_url"], entry) for entry in feed["entries"]]

    response = await signed_get(
        session, config["public_key_url"], config["private_key"], config["outbox_url"]
    )
    data = json.loads(await response.text())

    ids_in_outbox = [item["object"]["id"] for item in data["orderedItems"]]
    ...
    async with aiohttp.ClientSession() as session:
        async with asyncio.TaskGroup() as tg:
            for account_name, account_config in config.items():
                tg.create_task(update_account(session, account_config))
